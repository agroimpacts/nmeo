...
dev1 <- ArableClient(device = "A000667", measure = "daily", 
                     ...
                     password = "<pw>",  # replace this w/ actual p
                     tenant = "clark")
# a little cleaning
...
p1 <- ggplot(devs) + geom_line(aes(datetime, precip, colour = device)) +
  mytheme()
p2 <- ggplot(devs) + geom_line(aes(datetime, NDVI, colour = device)) +
  mytheme()
...
devs <- rbindlist(dev_list)  # joint them into a single dataset
```
...
ggplot(devs) + geom_point(aes(long, lat, colour = device))
```
