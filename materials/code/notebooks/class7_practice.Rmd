...
Testing read in and plot of UDM2 image
```{r, eval=FALSE}
...
# Use gdal to reproject images to EPSG4326 and to a defined extent
ext <- c(-71.8101, 42.1193, -71.8041, 42.1242)
out1 <- substr(basename(planet_imgs_udm[[7]]), 1, 1000)
...
image_date <- substr(basename(planet1@file@name), 1, 1000) 
plot(planet1_udm[[c(1)]], scale = 1, zlim = c(0, 1), stretch = "lin", ## band 1 is clearness index
...
```{r, eval=FALSE}
pdf(paste0("Whittier_Planet_all.pdf"), onefile = T)
for (i in seq(length(planet1))) {
## add code here
